...
namespace SigSpec.Core
{
    public class SigSpecGenerator
    {
        ...
        private async Task<SigSpecOperation> GenerateOperationAsync(Type type, MethodInfo method, JsonSchemaGenerator generator, SigSpecSchemaResolver resolver, SigSpecOperationType operationType)
        {
            ...
            operation.ReturnType = returnType == null ? null : generator.GenerateWithReferenceAndNullability<SigSpecReturnType>(
            ...
            if (method.ReturnType.IsGenericType)
            {
                ...
                var returnGenericNullability = nic.Create(method.ReturnParameter);
                var generic = returnGenericNullability.GenericTypeArguments.First();

                operation.ReturnType.Nullable = generic.ReadState == NullabilityState.Nullable;
            }
